function toggleMenu() {
  ...
  if (!track || !slides.length || !nextBtn || !prevBtn || !dotsContainer) {
    return;
  }
...
const toggleBtn = document.getElementById('toggleBtn');
    ...
    if (audio) {
        // Set volume to a comfortable background level (40%)
        audio.volume = 0.4; 

        // 1. Check if there is a saved playback position from the previous page
        const savedTime = localStorage.getItem('musicPlaybackTime');
        if (savedTime) {
            audio.currentTime = parseFloat(savedTime);
        }

        // 2. Continuously save the current time as the music plays
        audio.addEventListener('timeupdate', () => {
            localStorage.setItem('musicPlaybackTime', audio.currentTime);
        });

        // 3. Try to play right away
        const playAudio = () => {
            audio.play().catch(() => {
                console.log("Autoplay blocked by browser. Waiting for user interaction...");
                
                // Fallback: If browser blocks it, play on the very first click on the page
 ...
 // FIXED TIME EVALUATOR ENGINE (12-Hour Format & Sunday Open)
        document.addEventListener("DOMContentLoaded", () => {
            ...
            if (todayRow) {
                ...
                todaysHoursText.textContent = "Closes 9:00 PM";
            }
